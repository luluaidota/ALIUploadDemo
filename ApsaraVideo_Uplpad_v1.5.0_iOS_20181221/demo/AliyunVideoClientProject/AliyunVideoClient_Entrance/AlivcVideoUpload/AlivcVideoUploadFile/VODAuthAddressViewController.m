...
@interface VODAuthAddressViewController ()
{
...
@implementation VODAuthAddressViewController
...
- (void)viewDidLoad {
    ...
    __weak VODAuthAddressViewController *weakSelf = self;
...
//    NSString *ossObject = [NSString stringWithFormat:@"uploadtest/%ld.ios.demo.mp4", pos];
    ...
    [[GLUploadDataManager shareInstance] getVODCredentialsCompleteHandle:^(BOOL isSuccess) {
        if (isSuccess) {
            [[GLVODUpload sharedInstance] uploadFileWithFilePath:filePath videoInfo:vodInfo];
